...
import com.survivor.app.ui.screens.GridScreen
...
import com.survivor.app.ui.screens.PlannerScreen
...
private val tabs = listOf(
    ...
    Tab(Routes.PLANNER, "Planner", Icons.Filled.CalendarMonth),
    Tab(Routes.GRID, "Grid", Icons.Filled.GridOn),
...
fun SurvivorNavHost(vm: AppViewModel) {
    ...
    Scaffold(
        topBar = {
            Column {
                TopAppBar(
                    title = { Text(tabs.firstOrNull { it.route == current }?.label?.let { if (it == "Dashboard") "Survivor Optimizer" else it } ?: titleFor(current)) },
                    ...
                    actions = {
                        ...
                        DropdownMenu(expanded = menuOpen, onDismissRequest = { menuOpen = false }) {
                            ...
                            DropdownMenuItem(text = { Text("Optimize Season Path") }, onClick = { menuOpen = false; vm.recomputeNow(); go(Routes.PLANNER) })
        ...
        bottomBar = {
            NavigationBar {
                tabs.forEach { t ->
                    val selected = current == t.route || (t.route == Routes.MORE && current in setOf(Routes.PICKS, Routes.INPUTS, Routes.SETTINGS, Routes.SIMULATION, Routes.GUIDE))
        ...
        NavHost(nav, startDestination = Routes.DASHBOARD, Modifier.padding(padding)) {
            ...
            composable(Routes.PLANNER) { PlannerScreen(vm) }
            composable(Routes.GRID) { GridScreen(vm) }
