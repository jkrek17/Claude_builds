...
- [Betting: line shopping, model edge, staking, ledger](docs/BETTING.md)
...
The bottom bar has five destinations - Home, Rankings, Season, Bets, More - so the whole app is always at
most two taps away. Picks lives in More (recording/changing the current week's pick is also one tap from
Home's hero card).

| Screen | Purpose |
|---|---|
| **Home** (Dashboard) | A status strip (week, strike pips, teams used, data freshness); the hero recommendation card (team and opponent logos, win %, grade chip, spread/moneyline/source, the "Record for Week N" button or a Locked state with a Change link); a **Decision robustness** row from `RobustPlanner` ("Chosen in N% of 150 line scenarios", a Toss-up chip below 60%, and a note when a different team is more robust on average); an expandable **Why this pick** (why safe / why now / what we give up, plus the future-value warning); three expandable **Alternatives**; a **Top 10 this week** list linking to Rankings; a **Season outlook** card (P(win the pool) for your pool size, expected pool-end week, P(0 losses), P(≤1 loss), expected weeks alive, and the route as a horizontally-scrolling strip of team-logo chips with win % and a lock marker); and, only when the week's single best-scoring side is Strong tier (Bet Score ≥ 76), a small **Bets** teaser linking to the Bets tab (Home is only ever about the survivor pick). First run shows a welcome screen with a pool-size and strategy quick setup before the one **Download NFL data** button. |
...
| **Bets** | A graded board, priced off the same market data, never feeding back into the survivor pick. Every scheduled game of the current week appears on all three markets (moneyline, spread, total), both sides, each with a 0-100 **Bet Score**, letter grade and rank within its market - blending a real line-shopping edge (best book vs. no-vig consensus) with a small, clearly-labeled model-vs-market share. All/Moneyline/Spread/Total tabs, a **Top plays** list, and a Cards/Table toggle like Rankings; tap any row for the full breakdown (every score component, fair vs. best price, model probability and break-even, book disagreement, line movement) and a "Record bet" dialog per side. A **Ledger** of every recorded bet - graded automatically from final scores - with totals, a split by market, and per-bet CLV. |
| **More** | Picks, Weekly Inputs, Model Settings, Simulation, How to use and About, each with an icon and one-line description. About shows the app version and every data timestamp. |
| **More → Picks** | Record or change the week's pick from the available teams, shown as selectable rows with logos (used teams are hidden). The Used Teams list shows week, opponent, result chip and strike status. Results are automatic. |
...
| **More → Model Settings** | Four groups: **Your pool** (entries, payout-split assumption, field average win probability, and a plain-language line on what the pool size means for the model); **Betting** (bankroll, Kelly multiplier, max stake %, minimum edge for each signal, include totals); **Strategy** (route objective radio cards, horizon weight when Blended, ownership strategy preset, current-week override); **Advanced model parameters**, collapsed by default, with every weight (including total sigma) and a Restore defaults button. |
