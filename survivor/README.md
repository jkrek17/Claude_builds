...
| **Bets** | A separate module priced off the same market data, never feeding back into the survivor pick. **Line-shopping edges**: a book's price vs. the rest of the market's own no-vig consensus, ranked by EV, with best book/price, fair price, an EV chip, a suggested Kelly stake and a "Record bet" dialog. **Model disagreements**: this engine's own probability vs. the market, visibly marked Speculative. A market/min-EV filter row, and a **Ledger** of every recorded bet - graded automatically from final scores - with totals, a split by signal and by market, and per-bet CLV. |
